...
public class McRonald {
   ...
   public static void mcRonald(int TIME, PrintWriter of) {
      ...
      **********************************/
      ...
      int wait = -1;
      boolean serviceArea = false;
      
      for (int x = 0; x < TIME; x++) {
      // Chance of customer arriving in the minute
         if (Math.random() <= CHANCE_OF_CUSTOMER) {
            c = new Customer(x, timeToOrderAndBeServed());
            if (!serviceArea) wait = c.getOrderAndBeServed();
            q.add(c);
            of.println("new customer min#" + x);
         }
       // Check if customer should be moved into service area  
         if (!serviceArea && !q.isEmpty()) {
            serviceArea = true;
            current = q.remove();
         }
       ...
       // Customer is done being served  
         if (wait == 1) {
            int arrival = current.getArrivedAt();
            int waitTime = (x - arrival);
            serviceArea = false;
            of.println("Customer#" + arrival + " leaves and his total wait time is " + waitTime);
            if (!q.isEmpty()) {
               current = q.remove();
               wait = timeToOrderAndBeServed();
               serviceArea = true;
            }
            else {
               current.setArrivedAt(-1);
               current.setOrderAndBeServed(-1);
               wait = -1;
            }
         ...
         if (q.size() > longestQueue) longestQueue = q.size();
         displayTimeAndQueue(q, x);
         of.println("ServiceArea#1 " + current + ":" + wait);
         of.println("\n");
      }
               
      int x = TIME;
      while (!q.isEmpty() || serviceArea) {
         if (!serviceArea && !q.isEmpty()) {
            serviceArea = true;
            current = q.remove();
         }
