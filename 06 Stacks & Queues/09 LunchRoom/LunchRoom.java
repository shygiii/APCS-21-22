...
public class LunchRoom {
   ...
   public static void displayTimeAndQueue(int min, PriorityQueue<Customer> q) { 
   ...
   public static void lunchRoom(int TIME, PrintWriter outfile) {
      ...
      *************************/
      ...
      String[] gradeLevels = new String[] {"Se", "Ju", "So", "Fr"};
      int[] totalCustomers = new int[4];
      int[] totalTime = new int[4];
      int[] longestWait = new int[4];
      double[] avgWait = new double[4];
      
      for (int x = 0; x < TIME; x++) {
      // Chance of customer arriving in the minute
         if (Math.random() <= CHANCE_OF_CUSTOMER) {
            Customer c = new Customer(x, timeToOrderAndBeServed());
            if (!isServing) wait = c.getOrderAndBeServed();
            q.add(c);
         }
       // Check if customer should be moved into service area  
         if (!isServing && !q.isEmpty()) {
            isServing = true;
            current = q.remove();
         }
       // Customer is done being served  
         if (wait == 1) {
            String currentsGrade = current.getGrade();
            int arrival = current.getArrivedAt();
            int waitTime = (x - arrival);
            isServing = false;
            if (currentsGrade.equals("Se")) {
               totalTime[0] += waitTime;
               totalCustomers[0]++;
               if (waitTime > longestWait[0]) longestWait[0] = waitTime;
            }
            if (currentsGrade.equals("Ju")) {
               totalTime[1] += waitTime;
               totalCustomers[1]++;
               if (waitTime > longestWait[1]) longestWait[1] = waitTime;
            }
            if (currentsGrade.equals("So")) {
               totalTime[2] += waitTime;
               totalCustomers[2]++;
               if (waitTime > longestWait[2]) longestWait[2] = waitTime;
            }
            if (currentsGrade.equals("Fr")) {
               totalTime[3] += waitTime;
               totalCustomers[3]++;
               if (waitTime > longestWait[3]) longestWait[3] = waitTime;
            }
            if (!q.isEmpty()) { 
               current = q.remove();
         ...
         displayTimeAndQueue(x, q);
      }      
      
      int x = TIME;
      while (!q.isEmpty() || isServing) {
         // Check if customer should be moved into service area  
         if (!isServing && !q.isEmpty()) {
            isServing = true;
            current = q.remove();
      ...
      avgWait[0] = (int)(1.0 * totalTime[0]/totalCustomers[0] * 10)/10.0;
      avgWait[1] = (int)(1.0 * totalTime[1]/totalCustomers[1] * 10)/10.0;
      avgWait[2] = (int)(1.0 * totalTime[2]/totalCustomers[2] * 10)/10.0;
      avgWait[3] = (int)(1.0 * totalTime[3]/totalCustomers[3] * 10)/10.0;
      ...
      outfile.println("Senior\t\t\t" + totalCustomers[0] + "\t\t\t" + longestWait[0] + "\t\t\t" + avgWait[0]);
      outfile.println("Junior\t\t\t" + totalCustomers[1] + "\t\t\t" + longestWait[1] + "\t\t\t" + avgWait[1]);
      outfile.println("Sophomore\t\t" + totalCustomers[2] + "\t\t\t" + longestWait[2] + "\t\t\t" + avgWait[2]);
      outfile.println("Freshman\t\t\t" + totalCustomers[3] + "\t\t\t" + longestWait[3] + "\t\t\t" + avgWait[3]);
   }
   ...
   public static class Customer implements Comparable<Customer> {
      ...
      public Customer(int a, int o) {
         String[] gradeLevels = new String[] {"Se", "Ju", "So", "Fr"};
         grade = gradeLevels[(int)(Math.random() * 4)];   
         arrivedAt = a;
         orderAndBeServed = o;
      }
      ...
      public void setOrderAndBeServed(int o) {
         orderAndBeServed = o;
      }
      ...
      public int compareTo(Customer obj) {
         ...
         int studentA = 0;
         int studentB = 0;
         for (int x = 0; x < gradeLevels.length; x++) {
            if (gradeLevels[x].equals(grade)) studentA = x;
            if (gradeLevels[x].equals(obj.getGrade())) studentB = x;
         }
