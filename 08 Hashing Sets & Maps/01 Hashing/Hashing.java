...
class HashtableChaining implements Hashtable {
   ...
   public HashtableChaining(int size) {
      ...
      for (int x = 0; x < size; x++) array[x] = new LinkedList();       
   }
   ...
   public boolean contains(Object obj) {
      ...
      if (!array[index].isEmpty()) {
         if ((array[index].getFirst() != null && (array[index].getFirst()).equals(obj))) {  //found it
            System.out.println("\t\t\t Found at index " + index);
            return true;
         }
            ...
            int ct = 0;
            boolean found = false;
            while (!found || ct < array[index].size()) {
               if (array[index].get(ct).equals(obj)) {
                  found = true;
                  System.out.println("\t\t\t Found at index " + index);
                  return true;
               }
